...
5. ⏳ **Slice 2 — Telegram approval loop (HITL):** after insert, `Prep Approval` builds a preview
   ...
   **VERIFIED 2026-06-20:** tapped Approve in Telegram → execution resumed → row flipped to `ready`
   with `reviewed_at` stamped. Full HITL round-trip works. (Reject path is symmetric — same node,
   `false` branch.)
6. ⏳ **Slice 3 — dashboard:** `frontend/index.html` reads `content_public` via anon key;
...
7. ⏳ **Polish:** README (pitch, architecture diagram, demo GIF slot) ✅, exported workflow JSON ✅,
   error-trigger workflow ✅ (`02_error_handler.json`, generated by `scripts/build-error-workflow.ps1`),
   repo About/topics ✅. Remaining: record demo GIF, pin repo on profile.

   **Wire up the error handler:** import `workflows/02_error_handler.json`, attach the Telegram
   credential, then in **each production workflow → Settings → Error Workflow**, select
   *02 Error Handler*. Test it by forcing a node to fail (e.g. a bad URL) and confirm the
   Telegram alert fires with the workflow name + failed node + execution link.
